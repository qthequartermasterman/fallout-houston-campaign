...
COMBAT_DICE = "CD"
TAG_SKILL = '<input type="checkbox" disabled checked>'

AttributeName = Literal["STR", "PER", "END", "CHA", "INT", "AGI", "LCK", "BODY", "MIND"]
SkillName = Literal["Athletics", "Barter", "Big Guns", "Energy Weapons", "Explosives", "Lockpick", "Medicine", "Melee Weapons", "Pilot", "Repair", "Science", "Small Guns", "Sneak", "Speech", "Survival", "Throwing", "Unarmed", "Melee"]
...
class Character(pydantic.BaseModel):
    ...
    _phys_dr_override: str | None = None
    _energy_dr_override: str | None = None
    _radiation_dr_override: str | None = None
    _poison_dr_override: str | None = None
    ...
    def dr_physical(self) -> int:
        return self._phys_dr_override or 0

    @property
    def dr_energy(self) -> int:
        return self._energy_dr_override or 0

    @property
    def dr_radiation(self) -> int:
        return self._radiation_dr_override or 0

    @property
    def dr_poison(self) -> int:
        return self._poison_dr_override or 0
    ...
    def validate_special(self) -> 'Character':
        special_sum = sum([self.special.strength, self.special.perception, self.special.endurance, self.special.charisma, self.special.intelligence, self.special.agility, self.special.luck])
        ...
        if self.type == Type.normal and special_sum != math.ceil(35+ self.level/2):
            raise ValueError(f"Normal characters must have a total of ceil(35 + level/2) SPECIAL points, but this character has {special_sum} points.")
        elif self.type == Type.notable and special_sum != math.ceil(42 + self.level/2):
            raise ValueError(f"Notable characters must have a total of ceil(42 + level/2) SPECIAL points, but this character has {special_sum} points.")
        elif self.type == Type.major and special_sum != math.ceil(49 + self.level/2):
            raise ValueError(f"Major characters must have a total of ceil(49 + level/2) SPECIAL points, but this character has {special_sum} points.")
        return self
    ...
    def validate_skills(self) -> 'Character':
        ...
        if len(tags) != expected_tag_skills:
            raise ValueError(f"{self.type.value} characters must have exactly {expected_tag_skills} tag skills, but this character has {len(tags)} tag skills.")
        for tag in tags:
            if tag[1].rank < 2:
                raise ValueError(f"Tagged skills for characters start at 2 free ranks, but this character has {tag[1].rank} ranks in {tag[0]}.")
        if total_skill_points != 2*len(tags) + self.special.intelligence:
            raise ValueError(f"Characters must have a total of extra skill points equal to INT ({self.special.intelligence}), but this character has {total_skill_points - 2*len(tags)} extra skill points ({total_skill_points} total).")
...
def render_hp_initiative_defense(character:Character) -> str:
    return f"""
...
def render_dr(character:Character) -> str:
    return f"""
<div class="dr">
    <table class="center single-row-table">
        <tr class="character-header">
...
def render_attacks(attacks:list[Attack]|None, character:Character) -> str:
    if attacks:
        attacks_strs = [render_attack(attack, character.get_target_number(attack.target_attr, attack.target_skill)) for attack in attacks]
        rows = "\n".join([f"<tr><td>{attack_str}</td></tr>" for attack_str in attacks_strs])
    else:
        rows = "<tr><td>None</td></tr>"
    return f"""
<div class="attacks">
    <table class="multi-row-table center">
        <tr class="character-header">
...
def render_special_abilities(special_abilities:list[SpecialAbility]) -> str:
    if special_abilities:
        special_abilities_strs = [render_special_ability(special_ability) for special_ability in special_abilities]
        rows = "\n".join([f"<tr><td>{special_ability_str}</td></tr>" for special_ability_str in special_abilities_strs])
    else:
        rows = "<tr><td>None</td></tr>"
    return f"""
...
def render_attack(attack:Attack, target_number: int) -> str:
    name_target_str = f"<b>{attack.name.upper()}</b> ({attack.target_attr} + {attack.target_skill} TN {target_number})"
    ...
    describer_str = ', '.join([s for s in [qualities_str, range_str, fire_rate_str] if s])
    return f" ■ {name_target_str}, {dmg_str}, {describer_str}"

def render_special_ability(special_ability:SpecialAbility) -> str:
    return f" ■ <b>{special_ability.name.upper()}</b> {special_ability.description}"
